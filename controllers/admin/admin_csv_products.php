<?php
    ...
    $qSelect = "SELECT * FROM $dbDatabase .`products` "; // query for selecting record
    $eSelect = mysqli_query($dbConnection, $qSelect); // executing the query

    if ($eSelect == true) {

        $nFile = fopen("../../csv_reports/products.csv", "wa+");

        fwrite($nFile,"Product Code,Product Name,Price,Quantity\n");
        while($rows = mysqli_fetch_assoc($eSelect)){
            fwrite($nFile, '"'.$rows['ProductCode'].'","'.$rows['ProductName'].'","'.$rows['ProductPrice'].'","'.$rows['ProductQuantity'].'"');
            fwrite($nFile, "\n");
            }
            fclose($nFile);
        }else{
    ...
    $mail->Subject = "Product Merch Report";
    $mail->Body = nl2br('
    This file is a report of our products
    ');
    $mail->addAttachment("../../csv_reports/products.csv");
    $mail->IsHTML(true);
